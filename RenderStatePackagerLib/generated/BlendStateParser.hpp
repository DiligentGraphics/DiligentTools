...
inline void Serialize(nlohmann::json& Json, const RenderTargetBlendDesc& Type, DynamicLinearAllocator& Allocator)
{
    if (!(Type.BlendEnable == RenderTargetBlendDesc{}.BlendEnable))
        Serialize(Json["BlendEnable"], Type.BlendEnable, Allocator);

    if (!(Type.LogicOperationEnable == RenderTargetBlendDesc{}.LogicOperationEnable))
        Serialize(Json["LogicOperationEnable"], Type.LogicOperationEnable, Allocator);

    if (!(Type.SrcBlend == RenderTargetBlendDesc{}.SrcBlend))
        Serialize(Json["SrcBlend"], Type.SrcBlend, Allocator);

    if (!(Type.DestBlend == RenderTargetBlendDesc{}.DestBlend))
        Serialize(Json["DestBlend"], Type.DestBlend, Allocator);

    if (!(Type.BlendOp == RenderTargetBlendDesc{}.BlendOp))
        Serialize(Json["BlendOp"], Type.BlendOp, Allocator);

    if (!(Type.SrcBlendAlpha == RenderTargetBlendDesc{}.SrcBlendAlpha))
        Serialize(Json["SrcBlendAlpha"], Type.SrcBlendAlpha, Allocator);

    if (!(Type.DestBlendAlpha == RenderTargetBlendDesc{}.DestBlendAlpha))
        Serialize(Json["DestBlendAlpha"], Type.DestBlendAlpha, Allocator);

    if (!(Type.BlendOpAlpha == RenderTargetBlendDesc{}.BlendOpAlpha))
        Serialize(Json["BlendOpAlpha"], Type.BlendOpAlpha, Allocator);

    if (!(Type.LogicOp == RenderTargetBlendDesc{}.LogicOp))
        Serialize(Json["LogicOp"], Type.LogicOp, Allocator);
...
inline void Deserialize(const nlohmann::json& Json, RenderTargetBlendDesc& Type, DynamicLinearAllocator& Allocator)
{
    if (Json.contains("BlendEnable"))
        Deserialize(Json["BlendEnable"], Type.BlendEnable, Allocator);

    if (Json.contains("LogicOperationEnable"))
        Deserialize(Json["LogicOperationEnable"], Type.LogicOperationEnable, Allocator);

    if (Json.contains("SrcBlend"))
        Deserialize(Json["SrcBlend"], Type.SrcBlend, Allocator);

    if (Json.contains("DestBlend"))
        Deserialize(Json["DestBlend"], Type.DestBlend, Allocator);

    if (Json.contains("BlendOp"))
        Deserialize(Json["BlendOp"], Type.BlendOp, Allocator);

    if (Json.contains("SrcBlendAlpha"))
        Deserialize(Json["SrcBlendAlpha"], Type.SrcBlendAlpha, Allocator);

    if (Json.contains("DestBlendAlpha"))
        Deserialize(Json["DestBlendAlpha"], Type.DestBlendAlpha, Allocator);

    if (Json.contains("BlendOpAlpha"))
        Deserialize(Json["BlendOpAlpha"], Type.BlendOpAlpha, Allocator);

    if (Json.contains("LogicOp"))
        Deserialize(Json["LogicOp"], Type.LogicOp, Allocator);
...
inline void Serialize(nlohmann::json& Json, const BlendStateDesc& Type, DynamicLinearAllocator& Allocator)
{
    if (!(Type.AlphaToCoverageEnable == BlendStateDesc{}.AlphaToCoverageEnable))
        Serialize(Json["AlphaToCoverageEnable"], Type.AlphaToCoverageEnable, Allocator);

    if (!(Type.IndependentBlendEnable == BlendStateDesc{}.IndependentBlendEnable))
        Serialize(Json["IndependentBlendEnable"], Type.IndependentBlendEnable, Allocator);
...
inline void Deserialize(const nlohmann::json& Json, BlendStateDesc& Type, DynamicLinearAllocator& Allocator)
{
    if (Json.contains("AlphaToCoverageEnable"))
        Deserialize(Json["AlphaToCoverageEnable"], Type.AlphaToCoverageEnable, Allocator);

    if (Json.contains("IndependentBlendEnable"))
        Deserialize(Json["IndependentBlendEnable"], Type.IndependentBlendEnable, Allocator);

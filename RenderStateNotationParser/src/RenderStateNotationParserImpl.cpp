    ...
    TBase{pRefCounters}
{
    ...
    std::unordered_set<std::string>                                 IncludeHandler;
    std::function<void(const RenderStateNotationParserCreateInfo&)> ParseJSON;
    ParseJSON = [this, &ParseJSON, &IncludeHandler](const RenderStateNotationParserCreateInfo& ParserCI) {
        String Source;
        ...
        for (auto const& Import : Json["Imports"])
        {
            ...
            auto Iter = IncludeHandler.find(Path);
            if (Iter == IncludeHandler.end())
            {
                ParseJSON({Path.c_str(), nullptr, ParserCI.pStreamFactory});
                IncludeHandler.insert(Path);
            }
        }
